...
3. **🎉 새 탭 자동 열림!**
4. **파일 선택 영역 클릭** → 방금 다운로드한 파일 선택
5. **"✨ UTF-8-sig로 변환하기" 버튼 클릭**
6. **완료!** `파일명_utf8sig.csv` 생성

### **수동 사용 (기존 파일)**
1. 확장 프로그램 아이콘 클릭 또는 새 탭에서 실행
2. 기존 CSV 파일을 드래그 앤 드롭
3. 변환 버튼 클릭
...
| **변환 완료 알림 표시** | 변환 완료 시 Chrome 알림 표시 | ✅ 켜짐 |
...
### **아키텍처**
```
Background Script → 다운로드 감지 → 새 탭 생성 → 변환 인터페이스
```
...
```
Copyright (c) 2025 CSV UTF-8-sig Converter
```
...
### **Q: 다른 브라우저에서도 작동하나요?**
A: 현재는 Chrome 전용입니다. Edge, Brave 등 Chromium 기반 브라우저에서는 작동합니다.
